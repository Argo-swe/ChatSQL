...
class TestPromptGeneration:
    def __init__(self):
        """
        Nelle impostazioni del server di LM Studio,
        è opportuno abilitare il flag GPU Offload.
        Questo per evitare che la CPU debba svolgere tutto il lavoro,
        diminuendo significativamente le prestazioni.
        ...
        self.index_manager.create_or_load_index(self.data_dict_name)
    ...
    def rewrite_user_request(self, user_request):
        completion = self.client.chat.completions.create(
            model="lmstudio-community/Meta-Llama-3-8B-Instruct-GGUF",
            messages=[
                {
                    "role": "user",
                    "content": "Just return this slightly rewritten sentence: "
                    + user_request,
                }
            ],
            temperature=0.7,
    ...
    def iterate_prompt_generation(self, iteration, user_request):
        ...
        prompt = self.index_manager.prompt_generator(
            self.data_dict_name, user_request, activate_log=False
        )
        log.write(self.get_query_sql(prompt) + "\n\n")
        # Rimuovo le ulime due righe dal prompt
        lines = prompt.split("\n")
        lines = lines[:-2]
        prompt = "\n".join(lines)
        for i in range(5):
            rewritten_sentence = self.rewrite_user_request(user_request)
            ...
            log.write(self.get_query_sql(complete_prompt) + "\n\n")
