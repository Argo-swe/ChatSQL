import pytest
from adapter.outcoming.embeddings.txtai.txtai_index_manager_adapter import TxtaiIndexManagerAdapter
from adapter.outcoming.embeddings.txtai.txtai_prompt_manager_adapter import TxtaiPromptManagerAdapter
...
def prompt_manager(mock_index_manager, mock_file_repository):
    ...
    return TxtaiPromptManagerAdapter(index_manager=mock_index_manager, file_repository=mock_file_repository)
...
def test_prompt_generator_successful(prompt_manager, mocker, mock_file_repository):
    ...
    mocker.patch.object(prompt_manager, '_TxtaiPromptManagerAdapter__get_tuples', return_value=([{'max_score': 0.9}], []))
    # Mock the __get_relevant_tuples method to return a non-empty list of relevant tuples and an empty log content
    mocker.patch.object(prompt_manager, '_TxtaiPromptManagerAdapter__get_relevant_tuples', return_value=([{'max_score': 0.9}], []))
    ...
    # Assertions
    assert "Mocked schema metadata" in result  # Verify that the result contains the mocked schema metadata
    assert "Sample request" in result  # Verify that the result includes the user request
    assert log_content is None  # Verify that no log content is returned (since logging is not activated by default)
...
def test_prompt_generator_no_relevant_results(prompt_manager, mocker):
    ...
    mocker.patch.object(prompt_manager, '_TxtaiPromptManagerAdapter__get_tuples', return_value=([], []))
    # Mock the __get_relevant_tuples method to return an empty list of relevant tuples and an empty log content
    mocker.patch.object(prompt_manager, '_TxtaiPromptManagerAdapter__get_relevant_tuples', return_value=([], []))

    # Call the prompt_generator method with the mocked dependencies
    result, log_content = prompt_manager.prompt_generator(dictionary_id, user_request)

    # Assertions
    assert "Sorry, the ChatBOT was unable to find any relevant results" in result  # Verify that the no results message is returned
    assert log_content is None  # Verify that no log content is returned (since logging is not activated by default)
...
def test_prompt_generator_with_logging(prompt_manager, mocker, mock_file_repository):
    ...
    mocker.patch.object(prompt_manager, '_TxtaiPromptManagerAdapter__get_tuples', return_value=([{'max_score': 0.9}], ["log content phase 1"]))
    # Mock the __get_relevant_tuples method to return a non-empty list of relevant tuples and log content for phase 2
    mocker.patch.object(prompt_manager, '_TxtaiPromptManagerAdapter__get_relevant_tuples', return_value=([{'max_score': 0.9}], ["log content phase 2"]))

    # Mock the extract_schema_metadata method of the file repository to return a specific string
    mock_file_repository.extract_schema_metadata.return_value = "Mocked schema metadata"

    # Call the prompt_generator method with logging activated
    result, log_content = prompt_manager.prompt_generator(dictionary_id, user_request, activate_log=True)

    # Assertions
    assert "Mocked schema metadata" in result  # Verify that the result contains the mocked schema metadata
    assert "Sample request" in result  # Verify that the result includes the user request
    assert "log content phase 1" in log_content  # Verify that the log content from phase 1 is included
    assert "log content phase 2" in log_content  # Verify that the log content from phase 2 is included
...
def test_prompt_generator_empty_user_request(prompt_manager, mocker):
    dictionary_id = 1
    user_request = ""  # Empty request to simulate the scenario where the user provides no input.
...
def test_prompt_generator_different_language(prompt_manager, mocker, mock_file_repository):
    dictionary_id = 1
    user_request = "Sample request"
    ...
    result, log_content = prompt_manager.prompt_generator(dictionary_id, user_request, lang=lang)
    ...
    assert f"Answer in {lang}." in result  # Ensure the language setting is reflected in the output
...
def test_prompt_generator_different_dbms(prompt_manager, mocker, mock_file_repository):
    ...
    result, log_content = prompt_manager.prompt_generator(dictionary_id, user_request, dbms=dbms)
    ...
    assert f"Convert user request to a suitable SQL query for {dbms}." in result  # Ensure the DBMS setting is reflected in the output


"""Test for file repository failure handling"""

def test_prompt_generator_file_repository_failure(prompt_manager, mocker, mock_file_repository):
    dictionary_id = 1
    user_request = "Sample request"
    ...
    mock_file_repository.extract_schema_metadata.side_effect = Exception("Error extracting schema metadata")
    ...
    try:
        ...
        assert "Error" in result  # This assertion checks if an error message is returned.
...
def test_get_tuples_sql_query_and_search_execution(mocker, prompt_manager):
    ...
    mock_search = mocker.patch.object(prompt_manager._index_manager.get_embeddings(), 'search', return_value=[{'table_name': 'SampleTable'}])

    # Call the __get_tuples method
    tuples, log_content = prompt_manager._TxtaiPromptManagerAdapter__get_tuples(user_request, activate_log)
    ...
    actual_sql_query = normalize_sql_query(mock_search.call_args[0][0])  # The first argument in the call to search
    ...
    mock_search.assert_called_once_with(mock_search.call_args[0][0], limit=query_limit * 10, parameters={"x": user_request})
...
def test_get_tuples_logging_activation(mocker, prompt_manager):
    ...
    mock_log = mocker.patch.object(prompt_manager._debug_manager, 'semantic_search_log', return_value=["Log content"])
...
def test_get_tuples_empty_search_results(mocker, prompt_manager):
    ...
    mock_search = mocker.patch.object(prompt_manager._index_manager.get_embeddings(), 'search', return_value=[])
...
def test_get_relevant_tuples_keep_high_scores(mocker, prompt_manager):
    ...
    tuples = [
        {"table_name": "Table1", "max_score": 0.5},  # This tuple should be kept because its score >= 0.45
        {"table_name": "Table2", "max_score": 0.6},  # This tuple should also be kept due to a high score
        {"table_name": "Table3", "max_score": 0.4}   # This tuple should be kept due to scoring distance
    ]
    activate_log = False  # Logging is disabled for this test

    # Call the __get_relevant_tuples method
    relevant_tuples, log_content = prompt_manager._TxtaiPromptManagerAdapter__get_relevant_tuples(tuples, activate_log)

    # Assert that all tuples are kept
    assert relevant_tuples == [
        {"table_name": "Table1", "max_score": 0.5},
        {"table_name": "Table2", "max_score": 0.6},
        {"table_name": "Table3", "max_score": 0.4}  # This should be kept due to scoring distance
    ]
...
def test_get_relevant_tuples_keep_based_on_scoring_distance(mocker, prompt_manager):
    ...
    tuples = [
        ...
        {"table_name": "Table2", "max_score": 0.4},  # This tuple should be kept due to scoring distance
        {"table_name": "Table3", "max_score": 0.1}   # This tuple should be discarded due to a large scoring distance
    ]
    activate_log = False  # Logging is disabled for this test
...
def test_get_relevant_tuples_with_logging(mocker, prompt_manager):
    ...
    mock_log = mocker.patch.object(prompt_manager._debug_manager, 'semantic_search_log_custom_algorithm', return_value=["Log entry 1", "Log entry 2"])
    ...
    mock_log.assert_called_once_with(relevant_tuples, tuples)  # Ensure the logging method was called correctly
