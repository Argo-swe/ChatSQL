...
def test_semantic_search_log_empty_tuples(txtai_debug_manager):
    ...
    assert "[ChatSQL] [DEBUG] - Details of the prompt generation process.\n" in result[0]
...
def test_semantic_search_log_single_tuple(txtai_debug_manager, mock_index_manager):
    ...
    assert result[10].strip() == "Description of the most relevant column: Sample column description"
    assert result[11].strip() == "Ranking of the most relevant terms in the description:"
    assert result[12].strip() == "term1: 0.8"
    assert result[13].strip() == "term2: 0.7"

    # Verify the correct call to the mock
    mock_index_manager.get_embeddings().explain.assert_any_call(user_request, ["Sample text"], limit=1)
    mock_index_manager.get_embeddings().explain.assert_any_call(user_request, ["Sample column description"], limit=1)
...
def test_semantic_search_log_multiple_tuples(txtai_debug_manager, mock_index_manager):
    ...
    mock_index_manager.get_embeddings().explain.side_effect = [
        ...
        [{"tokens": [("term3", 0.6), ("term4", 0.5)]}],  # for "Sample column description 1"
        [{"tokens": [("term5", 0.4), ("term6", 0.3)]}],  # for "Sample text 2"
        [{"tokens": [("term7", 0.2), ("term8", 0.1)]}]   # for "Sample column description 2"
    ]
    ...
    assert result[8].strip() == "term1: 0.8"
    assert result[9].strip() == "term2: 0.7"
    assert result[10].strip() == "Description of the most relevant column: Sample column description 1"
    assert result[11].strip() == "Ranking of the most relevant terms in the description:"
    ...
    assert result[17].strip() == "Ranking of the most relevant terms in the description:"
    assert result[18].strip() == "term5: 0.4"
    assert result[19].strip() == "term6: 0.3"
    assert result[20].strip() == "Description of the most relevant column: Sample column description 2"
    assert result[21].strip() == "Ranking of the most relevant terms in the description:"
    assert result[22].strip() == "term7: 0.2"
    assert result[23].strip() == "term8: 0.1"

    # Verify the correct call to the mock
    mock_index_manager.get_embeddings().explain.assert_any_call(user_request, ["Sample text 1"], limit=1)
    mock_index_manager.get_embeddings().explain.assert_any_call(user_request, ["Sample column description 1"], limit=1)
    mock_index_manager.get_embeddings().explain.assert_any_call(user_request, ["Sample text 2"], limit=1)
    mock_index_manager.get_embeddings().explain.assert_any_call(user_request, ["Sample column description 2"], limit=1)
...
def test_semantic_search_log_custom_algorithm_no_tuples(txtai_debug_manager):
    ...
    result = txtai_debug_manager.semantic_search_log_custom_algorithm(relevant_tuples, tuples)
...
def test_semantic_search_log_custom_algorithm_all_kept(txtai_debug_manager):
    ...
    assert "The table Table1 is kept because it has a sufficiently high score." in result[2]
    assert "The table Table2 is kept because it has a sufficiently high score." in result[3]
    assert "The table Table3 is kept because it has a sufficiently high score." in result[4]
...
def test_semantic_search_log_custom_algorithm_some_discarded(txtai_debug_manager):
    ...
    assert "The remaining tables are discarded because the score is not high enough and the score difference with the previous tables is greater than 0.25." in result[3]
...
def test_semantic_search_log_custom_algorithm_first_underscore_table(txtai_debug_manager):
    ...
    relevant_tuples = [
        {"table_name": "Table1", "max_score": 0.4},  # Table1 should be kept due to scoring_distance <= 0.25
        {"table_name": "Table2", "max_score": 0.35}  # Table2 should also be kept for the same reason
    ]
    # In this test, the tuples list is identical to relevant_tuples
    tuples = relevant_tuples
    ...
    assert "The table Table1 is kept because the score difference with the previous table is less than 0.25." in result[2]
    
    # Assert that the second table is also kept due to a similar score difference
    assert "The table Table2 is kept because the score difference with the previous table is less than 0.25." in result[3]
